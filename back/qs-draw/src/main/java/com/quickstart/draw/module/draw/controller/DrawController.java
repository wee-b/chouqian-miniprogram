...
public class DrawController {
    ...
    @SentinelResource(
            value = SentinelResourceConstants.DRAW_DETAIL,
            entryType = EntryType.IN,
            blockHandler = "getDetailDrawBlockHandler",
            fallback = "getDetailDrawFallback",
    ...
    public ResponseDTO<DrawVO> getDetailDrawBlockHandler(Long drawId, BlockException ex) {
        log.warn("Sentinel block draw detail, drawId={}, reason={}", drawId, ex.getClass().getSimpleName());
        return ResponseDTO.error(ErrorCode.TOO_MANY_REQUESTS, "查询人数较多，请稍后刷新");
    }

    public ResponseDTO<DrawVO> getDetailDrawFallback(Long drawId, Throwable ex) {
        log.error("Sentinel fallback draw detail, drawId={}", drawId, ex);
        return ResponseDTO.error(ErrorCode.INTERNAL_SERVER_ERROR, "抽签详情查询失败，请稍后刷新");
    }
    ...
    @SentinelResource(
            value = SentinelResourceConstants.DRAW_STATISTICS,
            entryType = EntryType.IN,
            blockHandler = "queryStatisticsBlockHandler",
            fallback = "queryStatisticsFallback",
            exceptionsToIgnore = BusinessException.class
    )
    ...
    public ResponseDTO<DrawStatisticsVO> queryStatisticsBlockHandler(BlockException ex) {
        log.warn("Sentinel block draw statistics, reason={}", ex.getClass().getSimpleName());
        return ResponseDTO.error(ErrorCode.TOO_MANY_REQUESTS, "统计数据繁忙，请稍后刷新");
    }

    public ResponseDTO<DrawStatisticsVO> queryStatisticsFallback(Throwable ex) {
        log.error("Sentinel fallback draw statistics", ex);
        return ResponseDTO.error(ErrorCode.INTERNAL_SERVER_ERROR, "统计数据查询失败，请稍后刷新");
    }
